...
% explaination of side information
...
% eplaination of knowledge graph
...
This thesis constructed a heterogeneous graph to model the relationship between users and repositories by using Deep Graph Library (DGL) \cite{wang2019dgl}. The nodes of the graph are users and repositories. The table \ref{tab:edge_types} shows 4 types of edges in the graph. "watch" means a user watches a repository. "star" means a user stars a repository. "fork" means a user forks a repository. "own" means a user is the owner of a repository. 
...
Since edges in graphs created by the DGL library have directions. This thesis added 4 types of reverse relationships that are list in the table \ref{tab:reverse_edge_types}. They are presented in the graph as the reverse edges for the aforementioned 4 types of edges. This is to make the graph undirected.
...
The 18 features for repositories are shown in table \ref{tab:repo_features}.
...
\section{Training, validation and testing sub graphs}
...
\section{Negative edges sampling}
...
The learned representation vectors should be similar for nodes who have positive edges and dissimilar for those who have negative edges.

\section{Recommendation}
The recommendation process uses the learned representation vectors of users and repositories to calculate the similarity score between them. The recommender system then recommends the top $K$ most similar repositories to users.
...
To make the neural network learn to output those representations, this thesis used a hinge loss function as shown in the equation \ref{eq:loss_function}.
...
\section{Training}
The KGNN was trained with the PyTorch library in 100 epochs. The figure \ref{fig:training_results} shows that the loss stays stable at around 0.027 when the epoch is around 60.

...
\begin{center}
    ...
    \hline
    Literature & Domain & Method & Inputs & Outputs & Sparse data & Cold start \\
    \hline
    \cite{shao_paper2repo_2020} & Repositories recommednation for papers & Train a constrict graph GCN model to constrict the embeddings of papers and repositories into the same space and use a tag encoding to address the problem of cold start amd data sparcity. & A list of papers & A list of repositories & Y & Y \\
    \hline
    \cite{zhang_detecting_2017} & Java related repositories recommendation for users & Measure the similarity between repositories based on the TF-IDF ofreadme files and the time gap between the starred repositories, then recommend the most similar repositories. & A list of repositories and a query repository & A list of k similar repositories & N/A & N/A \\
    \hline
    \cite{inka_open_2018} & Github repositories recommendation for users & Measure the similarity between repositories basedon topics (binary vector), programming langauges (binary vector), and readme files (TF-IDF), then recommend the most similar repositories. & A repository & A list of recommended repositories & N/A & N/A  \\
    \hline
    \cite{jiang_open_2017} & Github recommendation for users & Model the relationship  between users's preference and repositories with a OOCF model, then predict ratings by using the trained model. & Users and Repositories & Recommended repositories for each user & N/A & N/A \\
    \hline
    \cite{xu_repersp_2017} & Github repositories recommendation for users & Represent the user's behaviour in the form a user-repository matrix, then calculate the similarity (TF-IDF and cosine distance) of repositories by considering their documents and source code, then generate and recommend the top K similar repositories. & Users and repositories & Recommended repositories for each user & N/A & N/A \\
    \hline
    \cite{sun_personalized_2018} & Github repositories recommendation for users & Represent the users' behaviour in the form of a matrix UP(user, repository), then calculate the similarity between repositories using documents and source code (calcaulate TF-IDF vectors), then epresent the similarity of repositories as a form of similarity matrix, then the most similar repositories & Users and repositories & Recommended repositories for each user & N/A & N/A \\
    \hline
